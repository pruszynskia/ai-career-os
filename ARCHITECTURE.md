...
- `profiles.evidence` (jsonb, ADR-017) is the verified claim base every generator draws on: a `claims` array (each `id`, `kind`, `text`, `sourceRef`, `state`, `riskLevel`, `note`, `metric`), plus `neverInclude`/`alwaysIncludeWhenRelevant` generation rules. Additive beside `skills`/`experience`/`projects`/`score`. `tailor-cv`, `cover-letter`, `recruiter-message` (TASK-081) and `linkedin-posts` (TASK-087) read it via `src/shared/ai/prompts/generation-contract.ts`'s `serializeEvidenceBase`, compose the shared `generationContractFragment` into their system prompts, and have every generation's `claimsUsed` checked by `src/shared/ai/claim-validator.ts` before it is persisted or returned. `posts.claims_used` (`text[]`, TASK-087) stores those ids so the post list can flag a claim reused across recent/scheduled posts.
